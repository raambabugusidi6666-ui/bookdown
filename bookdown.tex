...
\includegraphics[keepaspectratio,]{bookdown_files/images/by-nc-sa.png}\\
...
## [1] knitr_1.51      shiny_1.14.0    htmltools_0.5.9
## [4] rmarkdown_2.31  bookdown_0.47.2 miniUI_0.1.2   
## [7] tools_4.6.1
\end{verbatim}
...
This syntax is based on Pandoc's \href{https://pandoc.org/MANUAL.html\#divs-and-spans}{fenced \texttt{Div} blocks} and can already be used in any R Markdown document to write \href{https://yihui.org/rmarkdown-cookbook/custom-blocks.html}{custom blocks.} \textbf{Bookdown} only offers special handling for theorem and proof environments. Since this uses the syntax of Pandoc's Markdown, you can write any valid Markdown text inside the block.
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{bookdown_files/figure-latex/multi-plots-1} \includegraphics[width=0.5\linewidth]{bookdown_files/figure-latex/multi-plots-2} \caption{Two plots placed side by side.}\label{fig:multi-plots}
\end{figure}
...
\FunctionTok{url}\KeywordTok{:}\AttributeTok{ }\StringTok{\textquotesingle{}https\textbackslash{}://example.github.io/awesome/\textquotesingle{}}
...
\FunctionTok{date}\KeywordTok{:}\AttributeTok{ }\StringTok{"2026{-}07{-}08"}
...
\FunctionTok{url}\KeywordTok{:}\AttributeTok{ https://example.github.io/bookdown{-}demo}
...
\AttributeTok{    }\FunctionTok{primary}\KeywordTok{:}\AttributeTok{ }\StringTok{"\#0d6efd"}
\end{Highlighting}
\end{Shaded}
...
\AttributeTok{  }\FunctionTok{theme}\KeywordTok{:}
\AttributeTok{    }\FunctionTok{primary}\KeywordTok{:}\AttributeTok{ }\StringTok{"\#0d6efd"}
\AttributeTok{    }\FunctionTok{base\_font}\KeywordTok{:}
...
For LaTeX output, only the content of these blocks will be shown with no colored outline as for HTML. It is up to the user to define the appearance of these blocks for LaTeX output using custom environments. See the \href{https://yihui.org/rmarkdown-cookbook/custom-blocks.html}{\emph{R Markdown Cookbook}} for a how-to.
...
\section{Posit Connect Cloud}\label{rstudio-connect}

In theory, you can render the book by yourself and publish the output anywhere you want. For example, you can host the HTML files on your own web server. We have provided a function \texttt{publish\_book()} in \textbf{bookdown} to make it very simple to upload your book to \href{https://connect.posit.cloud}{Posit Connect Cloud},\index{Posit Connect Cloud} a free hosting service for R Markdown documents, Shiny applications, and more. Posit Connect Cloud is built on top of \href{https://posit.co/products/enterprise/connect/}{Posit Connect,}\index{Posit Connect} which is a Posit product that allows you to deploy a variety of R-related applications to a server.

You do not have to know much about Posit Connect to publish your book to Posit Connect Cloud. Basically you sign up at \url{https://connect.posit.cloud}, and the first time you try to run \texttt{bookdown::publish\_book()}\index{bookdown::publish\_book()}, you will be asked to authorize \textbf{bookdown} to publish to your Posit Connect Cloud account. In the future, you simply call \texttt{publish\_book()} again and \textbf{bookdown} will no longer ask for anything.
...
If you have set up your own Posit Connect server, you can certainly publish the book to that server instead of Posit Connect Cloud.
...
One service that you can utilize is GitHub Actions (\url{https://github.com/features/actions}).\index{GitHub Actions} It is free for public repositories on GitHub, and allows you to run arbitrary commands whenever you push to GitHub. These commands are specified in YAML workflow files under the \texttt{.github/workflows/} directory of your repository. You can use GitHub Actions to build your book and push the output to the \texttt{gh-pages} branch automatically.

A minimal GitHub Actions workflow to build and deploy a \textbf{bookdown} book to GitHub Pages might look like this:
...
\FunctionTok{name}\KeywordTok{:}\AttributeTok{ Build and deploy book}
...
\AttributeTok{      }\KeywordTok{{-}}\AttributeTok{ }\FunctionTok{uses}\KeywordTok{:}\AttributeTok{ r{-}lib/actions/setup{-}pandoc@v2}

\AttributeTok{      }\KeywordTok{{-}}\AttributeTok{ }\FunctionTok{name}\KeywordTok{:}\AttributeTok{ Install dependencies}
\AttributeTok{        }\FunctionTok{run}\KeywordTok{:}\AttributeTok{ Rscript {-}e \textquotesingle{}install.packages(c("bookdown"))\textquotesingle{}}

\AttributeTok{      }\KeywordTok{{-}}\AttributeTok{ }\FunctionTok{name}\KeywordTok{:}\AttributeTok{ Build book}
\AttributeTok{        }\FunctionTok{run}\KeywordTok{:}\AttributeTok{ Rscript {-}e \textquotesingle{}bookdown::render\_book("index.Rmd")\textquotesingle{}}
...
\AttributeTok{          }\FunctionTok{publish\_dir}\KeywordTok{:}\AttributeTok{ ./\_book}
\end{Highlighting}
\end{Shaded}

GitHub provides a built-in \texttt{GITHUB\_TOKEN} secret, so you do not need to create and encrypt a personal access token yourself. The \texttt{r-lib/actions} actions (\url{https://github.com/r-lib/actions}) provide convenient setup steps for R and Pandoc. You can find more complete examples in the \texttt{bookdown-demo} repository: \url{https://github.com/rstudio/bookdown-demo/}.
...
Besides publishing your book online, you can certainly consider publishing it with a publisher\index{publisher}. For example, this book was published with Chapman \& Hall/CRC, and there is also a free online version at \url{https://pkg.yihui.org/bookdown/} (with an agreement with the publisher). Another option that you can consider is self-publishing (\url{https://en.wikipedia.org/wiki/Self-publishing}) if you do not want to work with an established publisher. Pablo Casas has written two blog posts that you may find useful: \href{https://blog.datascienceheroes.com/how-to-self-publish-a-book/}{``How to self-publish a book''} and \href{https://blog.datascienceheroes.com/how-to-self-publish-a-book-customizing-bookdown/}{``How to self-publish a book: customizing bookdown''}.
...
\DocumentationTok{\#\# [1] \textquotesingle{}3.10\textquotesingle{}}
\end{Highlighting}
\end{Shaded}
...
\DocumentationTok{\#\#  pdfTeX 3.141592653{-}2.6{-}1.40.29 (TeX Live 2026)}
\DocumentationTok{\#\#  kpathsea version 6.4.2}
\DocumentationTok{\#\#  Copyright 2026 Han The Thanh (pdfTeX) et al.}
...
\DocumentationTok{\#\#  Compiled with libpng 1.6.55; using libpng 1.6.55}
\DocumentationTok{\#\#  Compiled with zlib 1.3.2; using zlib 1.3.2}
\DocumentationTok{\#\#  Compiled with xpdf version 4.06}
\end{Highlighting}
\end{Shaded}
