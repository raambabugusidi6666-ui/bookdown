--- 
title: "AWM/AME Technical Part -A"
author: "Gusidi Rambabu"
date: "`r Sys.Date()`"
...
description: "A guide to authoring books with R Markdown, including how to generate figures and tables, and insert cross-references, citations, HTML widgets, and Shiny apps in R Markdown. The book can be exported to HTML, PDF, and e-books (e.g. EPUB). The book style is customizable. You can easily write and preview the book in RStudio IDE or other editors, and host the book wherever you want (e.g. connect.posit.cloud)."
...
options(bookdown.post.latex = function(x) {
  # remove alt text inside \pandocbounded{} and this command itself
  sub('^\\\\pandocbounded\\{(.+?)alt=\\{.+?\\}(\\]\\{.+?\\})\\}($|\\\\\\\\)', '\\1\\2\\3', x)
})

# disable alt text for latex output since there'd be a latex error and I don't know how to fix it
if (knitr::is_latex_output()) knitr::opts_hooks$set(fig.alt = function(x) {
  x$fig.alt = NULL
  x
})
```
